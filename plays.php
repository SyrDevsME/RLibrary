...
<body>
	...
	<div class="menu">
			<a href="plays.php?q=new">New Plays</a>
			<a href="plays.php?q=popular">Most Popular</a>
	</div> 

	<div class="main-container">
	    <form class="search-bar" action="plays.php" method="get">
	    ...
	    <div class="entries">
			<?php
			    ...
			    $database = new PDO (
	      		...
	      		$sql = $database->prepare("SELECT * FROM `plays`");
			   	$sql->execute();
			   	...
			   	if (isset($_GET['query'])) {
					...
					$sql = $database->prepare("SELECT * FROM `plays` WHERE (`title` LIKE '%".$query."%')");
			   		$sql->execute();
			   		...
			   		if ($_GET['q'] == 'latest') {
			   			$sql = $database->prepare("SELECT * FROM `plays` WHERE year >= 2021");
			   			$sql->execute();
			   		}

			   		if ($_GET['q'] == 'new') {
			   			$sql = $database->prepare("SELECT * FROM `plays` WHERE year >= 2014");
			   			$sql->execute();

			   		}

			   		if ($_GET['q'] == 'best') {
			   			$sql = $database->prepare("SELECT * FROM `plays` WHERE res = 1080");
			   			$sql->execute();

			   		}

			   		if ($_GET['q'] == 'popular') {
			   			$sql = $database->prepare("SELECT * FROM `plays` WHERE views > 100");
			   			$sql->execute();
			   		}
