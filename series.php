...
<body>
	...
	<div class="menu">
			<a href="series.php?q=latest">Latest Episodes</a>
			<a href="series.php?q=new">New Series</a>
			<a href="series.php?q=best">Best Series</a>
			<a href="series.php?q=popular">Most Popular</a>
	    </div> 

		

	<div class="main-container">
	    <form class="search-bar" action="series.php" method="get">
	    ...
	    <div class="entries">
			<?php
			    ...
			    $database = new PDO (
	      		...
	      		$sql = $database->prepare("SELECT * FROM `series`");
			   	$sql->execute();
			   	...
			   	if (isset($_GET['query'])) {
					...
					$sql = $database->prepare("SELECT * FROM `series` WHERE (`title` LIKE '%".$query."%')");
			   		$sql->execute();
			   		...
			   		if ($_GET['q'] == 'latest') {
			   			$sql = $database->prepare("SELECT * FROM `series` WHERE year >= 2020");
			   			$sql->execute();
			   		}

			   		if ($_GET['q'] == 'new') {
			   			$sql = $database->prepare("SELECT * FROM `series` WHERE year >= 2014");
			   			$sql->execute();

			   		}

			   		if ($_GET['q'] == 'best') {
			   			$sql = $database->prepare("SELECT * FROM `series` WHERE res = 1080");
			   			$sql->execute();

			   		}

			   		if ($_GET['q'] == 'popular') {
			   			$sql = $database->prepare("SELECT * FROM `series` WHERE views > 100");
			   			$sql->execute();
			   		}
